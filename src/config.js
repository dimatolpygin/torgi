...
export const config = {
  ...
  timing: {
    ...
    // Упреждение выстрела (этап 20): за сколько мс до 00:00 отправлять заявку, чтобы
    // она ПРИЛЕТЕЛА на сервер сайта ближе к 00:00:00.000 (компенсация односторонней
    // сетевой задержки). Потолок = сетевой one-way (на BY ≈ 0.7 мс — мы в Минске рядом
    // с госЦОДом). Больше one-way → заявка прилетит до открытия даты → no_date, тогда
    // сработает безопасный фолбэк (повтор ровно в 00:00). ДЕФОЛТ 0 = выключено; включать
    // осознанно после калибровки границы по живым ночам. Отрицательные значения игнорим.
    sendAheadMs: Math.max(0, Number(process.env.SEND_AHEAD_MS || 0)),
    // Лёгкий путь для замера сетевого RTT на прогреве (статика — сеть без тяжёлой
  ...
  // Мультиконнект (этап 21): бороться с джиттером одного сокета, выстрелив ПЕРВОЙ
  // (гоночной) заявкой параллельно по K тёплым соединениям и взяв первый принятый
  // ответ. Все K делят одну сессию (кука gorodid), но на разных TCP-сокетах — как
  // браузер с несколькими соединениями. K=1 — выключено (текущее поведение). Лишние
  // приёмы (если сервер не срезал на лимите) гасятся отменой, чтобы в ЛК было ровно
  // нужное число мест. Место №1 гоночное; места №2..N — как раньше (с маскировкой).
  multiconnect: {
    k: Math.max(1, Number(process.env.MULTICONNECT_K || 1)),
  },
